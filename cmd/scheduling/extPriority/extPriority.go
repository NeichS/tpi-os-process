				...
				if colaProcesosListos.Peek().ExternalPriority > procesoEjecutando.ExternalPriority {
					if procesoEjecutando.PCB.TiempoTCP == TCP {
						...
						if procesoEjecutando.PCB.OperacionSOActual != "TCP" {
							...
							colaProcesosT.Enqueue(procesoEjecutando)
							listaProcesosSO = append(listaProcesosSO, procesoEjecutando)
							logs = append(logs, fmt.Sprintf("Tiempo %d: El proceso %s comienza a ejecutar su TCP porque hay un proceso con mayor prioridad. Prioridad proceso actual = %d | Prioridad proceso en listo = %d\n", unidadesDeTiempo, procesoEjecutando.PID, procesoEjecutando.ExternalPriority, colaProcesosListos.Peek().ExternalPriority))
						}
					}
				}
			}
			...
			if procesoEjecutandoSO != nil {
				...
				case "TIP":
					if TIP == 0 {
						...
						colaProcesosListos.Sort()
						procesoEjecutandoSO = nil
						if procesoEjecutando != nil && procesoEjecutando.BurstNeeded > procesoEjecutando.PCB.RafagasCompletadas {
